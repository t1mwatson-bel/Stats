...
def send_startup_message():
    ...
    msg += f"🔄 Версия: 7.1 (игрок + дилер)"
    send_message(msg)
    print(f"📤 Приветствие отправлено в канал", flush=True)

# =====================================================================
# ПАРСИНГ ИЗ КАНАЛА (ИГРОК + ДИЛЕР)
# =====================================================================
...
def check_results(history, all_messages):
    ...
    for entry in history:
        ...
        for i in range(max_games_to_check):
            ...
            # === ПРОВЕРЯЕМ И У ИГРОКА, И У ДИЛЕРА ===
            card_found = False
            all_cards = []
            ...
            print(f"   Проверка #N{game_to_check}: {len(all_cards)} карт (игрок+дилер)", flush=True)
            for card_str in all_cards:
                print(f"      Карта: {card_str}", flush=True)
                if card_str in predicted_cards:
                    card_found = True
                    print(f"   ✅ Найдена карта {card_str}", flush=True)
                    break
